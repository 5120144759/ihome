...
$(document).ready(function () {
    ...
    $(".display-mask").on("click", function (e) {
        ...
        total_page = 1;
        updateHouseData("renew");
        start_data = $('#start-date').val();
        end_data = $('#end-date').val();
        url = location.search;
        old_start_data = url.split('&')[2].split('=')[1];
        old_end_data = url.split('&')[3].split('=')[1];
        url = url.replace('sd='+ old_start_data, 'sd=' + start_data);
        url = url.replace('ed=' + old_end_data, 'ed=' + end_data);
        location.href = url;
    });
    ...
    $(".filter-item-bar>.filter-sort").on("click", "li", function (e) {
        if (!$(this).hasClass("active")) {
            ...
            old_key = search_url.split('&')[4].split('=')[1];
            location.href = location.search.replace('&sk=' + old_key, '&sk=' + sort_key)
        }
    });
    ...
    $.get('/house/my_search/' + search_url, function (data) {
        if (data.code == 200) {
            var swiper_html_script = template('swiper_html_script', {ohouse: data.house_list});
            $('.house-list').append(swiper_html_script)

        }
    });
    $.get('/house/area_facility/', function (data) {
        var swiper_html_script = template('swiper_script', {areas: data.areas});
            $('.filter-area').append(swiper_html_script)
    })
});